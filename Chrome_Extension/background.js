...
let parsons = async () => {
	let count = 0;//tracks current page of info pulled
	let status;//true if current fetch request was 200 level response, false otherwise
	let final_parsons = [];//final JSON var containing array of dicts, where each element contains dict with keys (name,link,details)
	let totatHits = 0;//total # of job listings
	let promise_arr;

	console.log();
	let origin = await fetch("https://jobsapi-google.m-cloud.io/api/job/search?callback=jobsCallback&pageSize=1&offset=24&companyName=companies%2Fedb4e875-81c8-4aa4-bb57-34d7c0f7fc5c&orderBy=posting_publish_time%20desc")
		.then(res => {
			status = res.ok;
			console.log("res value", status);
			return res;
	...
	// 	break;
	let src = await origin.text();
	const start = "jobsCallback(";
	const end = ")";
	const middleText = src.split(start)[1]?.split(end)[0];
	console.log("middle text", src.substring(13, src.length - 1))
	let obj = JSON.parse(src.substring(13,src.length-1));
	console.log("object for this endpoint", obj);
	totatHits = obj.totatHits;totatHits=parseInt(totatHits);
	console.log("count var", count++);
	// const parser = new DOMParser();
	// const htmlDoc = parser.parseFromString(src, 'text/html');

	origin = await fetch("https://jobsapi-google.m-cloud.io/api/job/search?callback=jobsCallback&pageSize="+24+"&offset=24&companyName=companies%2Fedb4e875-81c8-4aa4-bb57-34d7c0f7fc5c&orderBy=posting_publish_time%20desc")
		.then(res => {
			status = res.ok;
			console.log("res value", status);
			return res;
		});
	src = await origin.text();
	// console.log("middle text", src.substring(13, src.length - 1))
	obj = JSON.parse(src.substring(13, src.length - 1));
	console.log("object for this endpoint", obj);
	totatHits = obj.totatHits;
	let job_arr = obj.searchResults;
	...
	for(let i = 0; i < job_arr.length; i++) {
		...
		new_obj.link = link;
		new_obj.job_info = job_info;
		final_parsons.push(new_obj);
	}

	console.log("final parsons", final_parsons)
	...
	let formdata = new FormData();
	formdata.append("JSON_data", JSON.stringify(final_parsons));
	formdata.append("platform", "Parsons");
